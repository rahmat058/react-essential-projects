...
export function ShopApp() {
  ...
  const { catalogStatus, catalogError } = useAppSelector((state) => state.cart)
  ...
  return (
    <main className="mx-auto max-w-6xl flex-1 px-4 py-10 sm:px-6">
      ...
      >
        <h2 className="bg-gradient-to-r from-teal-600 via-teal-500 to-cyan-500 bg-clip-text text-3xl font-bold text-transparent sm:text-4xl">
          Shopping Cart
        </h2>
        <p className="mx-auto mt-3 max-w-xl text-sm text-slate-500 sm:text-base">
          Add items, update quantities, and watch subtotal, tax, shipping, and discounts recalculate
          via Redux selectors.
        </p>
      </motion.div>
